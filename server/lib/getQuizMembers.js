...
function getQuizMembers (client, quiz_id, callback) {
    ...
    var moduleQuery = 'SELECT scores.user_id, users.username, users.email, scores.score FROM scores INNER JOIN users ON scores.user_id = users.user_id WHERE quiz_id = $1;';
